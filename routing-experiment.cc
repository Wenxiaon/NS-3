...
RoutingExperiment::RoutingExperiment ()
  : port (9),
    ...
    m_averageTimeFile ("manet-routing.time.csv"),
...
RoutingExperiment::CheckThroughput ()
{
  ...
  out.close ();
  packetsReceived = 0;
  // every second doing statistics once
...
main (int argc, char *argv[])
{
  ...
  std::string AverageTimeFile = "manet-routing.time.csv";
...
*/
  for (int counts = 20; counts <= 200; counts+=5)
  {
      experiment.Run (nSinks, txp, CSVfileName, 5, counts);
  }
}
...
RoutingExperiment::Run (int nSinks, double txp, std::string CSVfileName, int protocol, int nodes)
{
  ...
  */
  ...
  //Set transmitting and receiving application
  ...
  for (int i = 0; i < nSinks; i++)
    {
      ...
      //Old routing transmitting packets
      ...
      factory.Set("NumPackets", UintegerValue(300));
