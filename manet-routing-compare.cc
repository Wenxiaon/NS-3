  ...
  while ((packet = socket->RecvFrom (senderAddress)))
    {
      ...
      NS_LOG_UNCOND ("Now the total packets received is " << packetsTotal);
...
{
  double averageTime = totalTime.GetSeconds()/packetsTotal;
  ...
  for (int i = 0; i < nSinks; i++)
    {
      ...
      sender->SetStartTime (Seconds(var->GetValue (100.0, 101.0)));
